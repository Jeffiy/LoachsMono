...
namespace Loachs.Data.Access
{
    public class Comment : IComment
    {
            ...
            int newId =
                Convert.ToInt32(
                    SqliteDbHelper.ExecuteScalar(
                        "select   [CommentId] from [loachs_comments]  order by [CommentId] desc limit 1"));
            return newId;
        }
        ...
        /// <returns></returns>
        public List<CommentInfo> GetCommentList(int pageSize, int pageIndex, out int totalRecord, int order, int userId,
            int postId, int parentId, int approved, int emailNotify, string keyword)
        {
            ...
            if (!string.IsNullOrEmpty(keyword))
            {
                condition +=
                    string.Format(
                        " and (content like '%{0}%' or author like '%{0}%' or ipaddress like '%{0}%' or email like '%{0}%'  or siteurl like '%{0}%' )",
                        keyword);
            }
            ...
            string cmdText = SqliteDbHelper.GetPageSql("[loachs_comments]", "[CommentId]", "*", pageSize, pageIndex,
                order, condition);
