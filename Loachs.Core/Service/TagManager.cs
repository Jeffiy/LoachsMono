...
namespace Loachs.Business
{
    ...
    {
        private static readonly ITag Dao = DataAccess.CreateTag();
        ...
        {
            return Tags.FirstOrDefault(t => !string.IsNullOrEmpty(slug) && t.Slug.ToLower() == slug.ToLower());
        }
        ...
        {
            return (from t in Tags where !string.IsNullOrEmpty(slug) && t.Slug.ToLower() == slug.ToLower() select t.TagId).FirstOrDefault();
        }
        ...
        {
            foreach (TagInfo t in Tags.Where(t => t.TagId == tagId))
            {
                return t.Name;
            }
            return string.Empty;
        ...
        public static List<TagInfo> GetTagList(string ids)
        {
            ...
            return (from str in tempids from tag in list where tag.TagId.ToString() == str select tag).ToList();
        }
